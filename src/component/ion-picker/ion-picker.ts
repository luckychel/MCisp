...
@Component({
   selector: 'ion-picker',
    ...
    <ion-input
        ...
        (tap)="(showResultsFirst || (showWhenTap && keyword.length > 0)) && getItems()"
        [(ngModel)]="keyword"
...
<ul *ngIf="!disabled && suggestions.length > 0 && showList">
    <li *ngFor="let suggestion of suggestions" (tap)="select(suggestion);$event.srcEvent.stopPropagation()">
        <ng-template
                ...
                [ngOutletContext]="
                  {attrs:{ data: suggestion, idModel:idModel, nameModel:nameModel, keyword: keyword, labelAttribute: dataProvider.labelAttribute }}"></ng-template>
    </li>
</ul>
<input type="text" [(ngModel)]="idModel" />
...
@Input() public idModel: any;
@Input() public nameModel: any;
@Input() public dataProvider: any;
@Input() public options: any;
@Input() public disabled: any;
@Input() public keyword: string;
@Input() public showResultsFirst: boolean;
@Input() public showWhenTap: boolean;
...
@HostListener('document:click', ['$event'])
    private documentClickHandler(event) {
        if ((this.searchbarElem && !this.searchbarElem._elementRef.nativeElement.contains(event.target)) || (!this.inputElem && this.inputElem._elementRef.nativeElement.contains(event.target))) 
        {
            this.hideItemList();
        }
    }
